...
import { getJWalletContract, parseJettonWalletDetails } from "./jetton-utils";
import fs from "fs";

import { WrappedSmartContract } from "./wrapped-smart-contract";
const api_key = process.env.TON_API_TOKEN;
export function getAccountTimeLockAddr(
  account: Address,
...
export async function getAccountJettonBalance(
  account: Address,
  jetton: Address
) {
  ...
  let jWalletCurrent = await WrappedSmartContract.create(
    Cell.fromBoc(states.code)[0],
    Cell.fromBoc(states.data)[0]
  );
...
export async function getLaunchpadInfo(launchpadAddr: Address) {
  ...
  let launchpad = await WrappedSmartContract.create(
    Cell.fromBoc(states.code)[0],
    Cell.fromBoc(states.data)[0]
  );
  const launchpadData = await launchpad.contract.invokeGetMethod(
    "get_info",
    []
  );
  return {
    ...
    sourceJetton:
      (launchpadData.result[2] as Slice).remainingBits > 2
        ? (launchpadData.result[2] as Slice).loadAddress()
        : null,
